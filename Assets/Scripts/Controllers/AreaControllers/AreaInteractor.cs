...
public class AreaInteractor : MonoBehaviour, IInteractable
{
    ...
    {
        if (!interactorsInside.Contains(other.transform)) return;
        ...
        if (stack != null)
        {
            // Sırayla delay ile bitiriyoruz, Tween kendi animasyonunu sürdürüyor
            StartCoroutine(stack.CompleteActiveAnimations(0.05f));
        }
    ...
    {
        if (!interactorsInside.Contains(interactor) || interactorBusy[interactor]) return;
    ...
    private IEnumerator SendToTrash(Transform interactor)
    {
        ...
        if (trashMachine == null) yield break;
